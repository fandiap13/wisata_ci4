<div class="modal" id="modaledit">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="<?= site_url('/admin/users/update_pengunjung'); ?>" method="POST" class="formsimpan">
        <?= csrf_field(); ?>
        <input type="hidden" name="id" value="<?= $pengunjung['id']; ?>">
        <div class="modal-body">
          <div class="form-group">
            <label for="email_user" class="form-label">Email</label>
            <input type="email" name="email_user" class="form-control" placeholder="Email..." value="<?= $pengunjung['email_user']; ?>">
            <div class="invalid-feedback error_email_user"></div>
          </div>
          <div class="form-group">
            <label for="nama_user" class="form-label">Nama</label>
            <input type="text" name="nama_user" class="form-control" placeholder="Nama..." value="<?= $pengunjung['nama_user']; ?>">
            <div class="invalid-feedback error_nama_user"></div>
          </div>
          <div class="form-group">
            <label for="telp_user" class="form-label">Telp</label>
            <input type="text" name="telp_user" class="form-control" placeholder="Telp..." value="<?= $pengunjung['telp_user']; ?>">
      ...
      success: function(response) {
        ...
        if (response.errors) {
          ...
          if (response.errors.telp_user) {
            ...
            $('input[name=error_telp_user]').removeClass('is-invalid');
            $('.error_error_telp_user').html('');
          }
        }
      },
